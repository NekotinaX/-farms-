...
AddEventHandler('esx_birne:hasEnteredMarker', function(zone)
	...
	if zone == 'BirnenSammler' then
		CurrentAction     = zone
		CurrentActionMsg  = _U('objekt_sammeln')
		CurrentActionData = {}
	elseif zone == 'BirnenVerarbeiter' then
		if kodaQTE >= 5 then
			CurrentAction     = zone
			CurrentActionMsg  = _U('objekt_verarbeiten')
			CurrentActionData = {}
		end
	elseif zone == 'BirnenVerkaeufer' then
...
AddEventHandler('esx_birne:hasExitedMarker', function(zone)
	CurrentAction = nil
	ESX.UI.Menu.CloseAll()

	if zone == 'BirnenSammler' then
		TriggerServerEvent('esx_birne:stopHarvestKoda')
	elseif zone == 'BirnenVerarbeiter' then
	TriggerServerEvent('esx_birne:stopTransformKoda')
	elseif zone == 'BirnenVerkaeufer' then
		TriggerServerEvent('esx_birne:stopSellKoda')
	end
end)
...
-- itemsy
RegisterNetEvent('esx_birne:ReturnInventory')
AddEventHandler('esx_birne:ReturnInventory', function(kodaNbr, kodapNbr, jobName, currentZone)
	kodaQTE			= kodaNbr
	koda_poochQTE	= kodapNbr
	myJob			= jobName
	TriggerEvent('esx_birne:hasEnteredMarker', currentZone)
end)
...
Citizen.CreateThread(function()
	while true do
		...
		if isInMarker and not hasAlreadyEnteredMarker then
			...
			TriggerServerEvent('esx_birne:GetUserInventory', currentZone)
		end

		if not isInMarker and hasAlreadyEnteredMarker then
			hasAlreadyEnteredMarker = false
			TriggerEvent('esx_birne:hasExitedMarker', lastZone)
		end
	end
end)
